...
export function ThemeToggle({
  ...
  const [theme, setTheme]     = useState<Theme>(globalThemeCtx.theme as Theme || defaultTheme);
  ...
  useEffect(() => {
    if (globalThemeCtx.theme && globalThemeCtx.theme !== theme) {
      setTheme(globalThemeCtx.theme as Theme);
    }
  }, [globalThemeCtx.theme]);
  ...
  const toggle = useCallback(() => {
    ...
    setTimeout(() => {
      setTheme(next);
      onThemeChange?.(next);
      globalThemeCtx.toggleTheme();

      setPhase("rising");
      setTimeout(() => setPhase("idle"), duration + 60);
    }, duration);
  }, [phase, theme, duration, onThemeChange, globalThemeCtx]);
  ...
  const btnStyle: CSSProperties = {
    ...
    width: buttonSize,
    height: buttonSize,
    borderRadius: "6px", // Modified from 50% to match app's aesthetic
    ...
    outline: "none",
    transition:
      "background 0.3s ease, color 0.3s ease, transform 0.15s ease, box-shadow 0.3s ease, border-color 0.3s ease",
